let pacientes = [
  {
    id: 1,
    nome: "João Silva",
    cpf: "123.456.789-00",
    dataNascimento: "15 05 1985",
    telefone: "(11) 98765-4321",
    endereco: "Rua das Flores, 123 - Apto 45, São Paulo - SP",
    convenio: "Amil Saúde",
    historico: "Acompanhamento de hipertensão, última consulta em 10/09/2025.",
    adulto: true,
  },
  {
    id: 2,
    nome: "Maria Oliveira",
    cpf: "987.654.321-11",
    dataNascimento: "22 10 1992",
    telefone: "(21) 99876-5432",
    endereco: "Avenida Atlântica, 500, Rio de Janeiro - RJ",
    convenio: "Unimed",
    historico:
      "Consulta de rotina, sem queixas. Exames laboratoriais solicitados.",
    adulto: true,
  },
  {
    id: 3,
    nome: "Pedro Souza",
    cpf: "111.222.333-44",
    dataNascimento: "03 02 2001",
    telefone: "(31) 97654-3210",
    endereco: "Rua Belo Horizonte, 789, Belo Horizonte - MG",
    convenio: "Bradesco Saúde",
    historico:
      "Tratamento para alergia sazonal. Medicamento receitado: anti-histamínico.",
    jovem: true,
  },
  {
    id: 4,
    nome: "Ana Santos",
    cpf: "444.555.666-77",
    dataNascimento: "29 07 1978",
    telefone: "(41) 96543-2109",
    endereco: "Rua do Comércio, 100, Curitiba - PR",
    convenio: "SulAmérica",
    historico: "Revisão pós-operatória de cirurgia no joelho, boa recuperação.",
    adulto: true,
  },
  {
    id: 5,
    nome: "Carlos Ferreira",
    cpf: "777.888.999-00",
    dataNascimento: "01 01 1965",
    telefone: "(51) 95432-1098",
    endereco: "Rua dos Andradas, 250, Porto Alegre - RS",
    convenio: "Particular",
    historico:
      "Avaliação cardiológica completa. Laudo do eletrocardiograma normal.",
    idoso: true,
  },
  {
    id: 6,
    nome: "Camila Dias",
    cpf: "555.444.333-22",
    dataNascimento: "12 03 1990",
    telefone: "(11) 91234-5678",
    endereco: "Rua das Palmeiras, 300, São Paulo - SP",
    convenio: "Amil Saúde",
    historico:
      "Consulta ginecológica de rotina, exames de Papanicolau e ultrassom solicitados.",
    adulto: true,
  },
  {
    id: 7,
    nome: "Ricardo Gomes",
    cpf: "222.333.444-55",
    dataNascimento: "08 11 1972",
    telefone: "(21) 92345-6789",
    endereco: "Rua Getúlio Vargas, 77, Niterói - RJ",
    convenio: "Bradesco Saúde",
    historico:
      "Acompanhamento de diabetes tipo 2. Ajuste na dose de medicação oral.",
    adulto: true,
  },
  {
    id: 8,
    nome: "Juliana Lima",
    cpf: "333.444.555-66",
    dataNascimento: "25 06 1988",
    telefone: "(31) 93456-7890",
    endereco: "Avenida Afonso Pena, 1500, Belo Horizonte - MG",
    convenio: "SulAmérica",
    historico: "Dor de cabeça recorrente. Encaminhada para neurologista.",
    adulto: true,
  },
  {
    id: 9,
    nome: "Fernando Costa",
    cpf: "666.777.888-99",
    dataNascimento: "19 04 1950",
    telefone: "(41) 94567-8901",
    endereco: "Rua Cândido de Abreu, 950, Curitiba - PR",
    convenio: "Unimed",
    historico: "Check-up anual, exames de sangue e urina em dia. Tudo normal.",
    idoso: true,
  },
  {
    id: 10,
    nome: "Patrícia Ribeiro",
    cpf: "999.000.111-22",
    dataNascimento: "30 09 2005",
    telefone: "(51) 95678-9012",
    endereco: "Rua Coronel Fernando Machado, 40, Porto Alegre - RS",
    convenio: "Particular",
    historico:
      "Vacinação de rotina, segunda dose da vacina contra HPV aplicada.",
    jovem: true,
  },
];

export default { pacientes };
